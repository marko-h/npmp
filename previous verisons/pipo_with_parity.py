...
if __name__ == "__main__":
    ...
    # Choose how bit errors are introduced into the system
    # MODE 1: Manual scheduled errors (for testing specific scenarios)
    # MODE 2: Thermal noise model (biologically realistic spontaneous flips)
    
    ERROR_MODE = 'THERMAL_NOISE'  
    # ERROR_MODE = 'MANUAL_INJECTION'
    
    # ===== MODE 1: MANUAL ERROR INJECTIONS =====
    # Define specific error windows: (bit_index, time_start, time_end)
    # To use manual injections: set ERROR_MODE = 'MANUAL_INJECTION' and uncomment items below
    ERROR_INJECTIONS = [
        (0, 405, 450),   # Flip bit 0 between t=405-450
        # (1, 250, 300),   # Flip bit 1 between t=250-300
    ]
    
    # ===== MODE 2: THERMAL NOISE MODEL =====
    # Biologically realistic PERSISTENT spontaneous state transitions
    # When a flip-flop spontaneously changes state, it STAYS changed (like real bistable circuits)
    # Parameters: choose percentage-based or multiplier-based via flag
    USE_PERCENTAGE_RATES = True   # True → use percentage settings below; False → use FLIP_RATE_BASE multiplier
    
    # Multiplier mode (used when USE_PERCENTAGE_RATES is False)
    FLIP_RATE_BASE = 0.5          # Base transition rate multiplier (higher = more frequent transitions)
    
    # Percentage mode (used when USE_PERCENTAGE_RATES is True)
    # Tuning guide:
    # - ERROR_RATE_BASE_PERCENT: baseline per-timepoint transition chance across all bits, regardless of stability.
    #   Increase for more global flips (e.g., 0.2 = 0.2%).
    # - ERROR_RATE_STABILITY_PERCENT: extra percent added near threshold; scales with instability via exp(-stability/THERMAL_NOISE).
    #   Increase to emphasize flips when Q ≈ not_Q (e.g., 8.0%).
    # - THERMAL_NOISE: larger → weaker stability effect; smaller → stronger threshold sensitivity.
    # - STABILITY_THRESHOLD: widen/narrow the region considered near-threshold.
    ERROR_RATE_BASE_PERCENT = 0.1       # Baseline flip probability per timepoint (percent)
    ERROR_RATE_STABILITY_PERCENT = 3.0  # Additional percent near threshold (percent)
    
    THERMAL_NOISE = 5.0         # Moderate sensitivity to instability
    STABILITY_THRESHOLD = 50.0   # Q-not_Q difference below which bits are unstable
    
    # Initialize corrupted signal tracking
    Q_corrupted = Q.copy()
    
    # Apply error injection based on selected mode
    if ERROR_MODE == 'MANUAL_INJECTION':
        # Original manual injection method
        for bit_idx, t_start, t_end in ERROR_INJECTIONS:
            mask = (T >= t_start) & (T < t_end)
            digital_out[mask, bit_idx] = 1 - digital_out[mask, bit_idx]  # Flip the bit
            Q_corrupted[mask, bit_idx] = 400 - Q[mask, bit_idx]  # Invert analog signal
            
    elif ERROR_MODE == 'THERMAL_NOISE':
        # Thermal noise model: PERSISTENT spontaneous state transitions (biologically realistic)
        # Unlike transient noise, when a flip-flop switches state, it STAYS in that new state
        # This models real bistable circuit behavior with hysteresis
        # np.random.seed(42)  # Commented out for true randomness each run

        # Targeted overall parity error percentage (approximate)
        TARGET_ERROR_PERCENT = 10.0
        TOLERANCE_PERCENT = 2.0
        MAX_ATTEMPTS = 4

        # Iteratively tune percentages to approach target error rate
        for attempt in range(MAX_ATTEMPTS):
            # Reset outputs to original (pre-error) state each attempt
            digital_out = original_digital_out.copy()
            Q_corrupted = Q.copy()
            # No seed: different random pattern each run

            # ONE BIT AT A TIME: Randomly select which bit gets errors
            # Calculate average flip probability across all bits to choose target bit
            bit_flip_probabilities = []
            for i in range(4):
                stability = np.abs(Q[:, i] - not_Q[:, i])
                if USE_PERCENTAGE_RATES:
                    baseline_prob = ERROR_RATE_BASE_PERCENT / 100.0
                    stability_prob = (ERROR_RATE_STABILITY_PERCENT / 100.0) * np.exp(-stability / THERMAL_NOISE)
                    flip_probability = np.full(len(T), baseline_prob) + stability_prob
                else:
                    flip_probability = np.full(len(T), FLIP_RATE_BASE * 0.0001)
                    stability_component = FLIP_RATE_BASE * 0.01 * np.exp(-stability / THERMAL_NOISE)
                    flip_probability += stability_component
                near_threshold = stability < STABILITY_THRESHOLD
                flip_probability[near_threshold] *= 5.0
                flip_probability = np.minimum(flip_probability, 1.0)
                bit_flip_probabilities.append(flip_probability.mean())
            
            # Randomly choose one bit weighted by its average flip probability
            bit_weights = np.array(bit_flip_probabilities)
            bit_weights = bit_weights / bit_weights.sum()  # normalize to probabilities
            chosen_bit = np.random.choice(4, p=bit_weights)
            
            # Apply flips ONLY to the chosen bit
            i = chosen_bit
            stability = np.abs(Q[:, i] - not_Q[:, i])

            # Construct flip probability per timepoint
            if USE_PERCENTAGE_RATES:
                baseline_prob = ERROR_RATE_BASE_PERCENT / 100.0
                stability_prob = (ERROR_RATE_STABILITY_PERCENT / 100.0) * np.exp(-stability / THERMAL_NOISE)
                flip_probability = np.full(len(T), baseline_prob) + stability_prob
            ...
            # Additional boost for bits very close to threshold
            near_threshold = stability < STABILITY_THRESHOLD
            flip_probability[near_threshold] *= 5.0
            ...
            # Generate random flip EVENTS (not continuous noise)
            random_values = np.random.random(len(T))
            flip_events = random_values < flip_probability

            # PERSISTENT FLIP MODEL: cumulative toggling
            cumulative_flips = np.cumsum(flip_events)
            persistent_flip_mask = (cumulative_flips % 2) == 1

            # Apply persistent flips to digital output and Q visualization
            digital_out[persistent_flip_mask, i] = 1 - digital_out[persistent_flip_mask, i]
            Q_corrupted[persistent_flip_mask, i] = 400 - Q[persistent_flip_mask, i]

            # Evaluate error rate after applying flips
            calculated_parity = (digital_out.sum(axis=1) % 2).astype(int)
            parity_error = (stored_parity != calculated_parity).astype(int)
            current_error_rate = parity_error.sum() / len(T) * 100.0

            # If within tolerance, stop tuning
            if abs(current_error_rate - TARGET_ERROR_PERCENT) <= TOLERANCE_PERCENT:
                print(f"[Tuning] Achieved error rate ~ {current_error_rate:.2f}% (target {TARGET_ERROR_PERCENT}%)")
                break

            # Otherwise scale percentages to move toward target (percentage mode only)
            if USE_PERCENTAGE_RATES:
                scale = TARGET_ERROR_PERCENT / max(current_error_rate, 0.001)
                # Limit aggressive changes per attempt
                scale = max(0.3, min(scale, 2.5))
                ERROR_RATE_BASE_PERCENT *= scale
                ERROR_RATE_STABILITY_PERCENT *= scale
                print(f"[Tuning] Adjusting rates by x{scale:.2f} → base={ERROR_RATE_BASE_PERCENT:.3f}%, stability={ERROR_RATE_STABILITY_PERCENT:.3f}%")
        else:
            # Tuning loop completed without hitting tolerance
            print(f"[Tuning] Final achieved error rate ~ {current_error_rate:.2f}% (target {TARGET_ERROR_PERCENT}%)")
    ...
    print(f"\n{'='*60}")
    print(f"ERROR INJECTION REPORT (Mode: {ERROR_MODE})")
    print(f"{'='*60}")
    if ERROR_MODE == 'THERMAL_NOISE':
        for i in range(4):
            bit_flips = np.sum(np.diff(digital_out[:, i]) != 0)
            print(f"  Bit {i}: {bit_flips} spontaneous flips detected")
    ...
    # Add corrupted signal (red line) if any errors detected
    # Show in MANUAL_INJECTION mode if this bit has scheduled errors, or always in THERMAL_NOISE mode
    show_corrupted = (ERROR_MODE == 'THERMAL_NOISE' or 
                     (ERROR_MODE == 'MANUAL_INJECTION' and any(bit_idx == i for bit_idx, _, _ in ERROR_INJECTIONS)))
    if show_corrupted and not np.array_equal(Q[:, i], Q_corrupted[:, i]):
